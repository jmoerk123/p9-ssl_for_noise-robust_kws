...
from utils.trainer_split_noise import train, evaluate
...
def training_pipeline(config):
    ...
    trainloader_student = get_loader(train_student_list, config, train=True)
    trainloader_teacher = get_loader(train_teacher_list, config, train=True)
    ...
    if config["hparams"]["scheduler"]["n_warmup"]:
        schedulers["warmup"] = WarmUpLR(optimizer, total_iters=len(trainloader_student) * config["hparams"]["scheduler"]["n_warmup"])

    if config["hparams"]["scheduler"]["scheduler_type"] is not None:
        total_iters = len(trainloader_student) * max(1, (config["hparams"]["scheduler"]["max_epochs"] - config["hparams"]["scheduler"]["n_warmup"]))
    ...
    train(model, optimizer, criterion, trainloader_student, trainloader_teacher, valloader, schedulers, config)

    #####################################
    # Final Test
    #####################################

    final_step = calc_step(config["hparams"]["n_epochs"] + 1, len(trainloader_student), len(trainloader_student) - 1)
