...
def train_single_batch(net: nn.Module, data_student: torch.Tensor, targets_student: torch.Tensor, data_teacher: torch.Tensor, targets_teacher: torch.Tensor, optimizer: optim.Optimizer, criterion: Callable, device: torch.device) -> Tuple[float, int]:
    ...
    data_student, targets_student = data_student.to(device), targets_student.to(device)
    data_teacher, targets_teacher = data_teacher.to(device), targets_teacher.to(device)

    optimizer.zero_grad()
    outputs_student = net(data_student)
    outputs_teacher = net(data_teacher)
    loss_student = criterion(outputs_student, targets_student)
...
def train(net: nn.Module, optimizer: optim.Optimizer, criterion: Callable, trainloader_student: DataLoader, trainloader_teacher: DataLoader, valloader: DataLoader, schedulers: dict, config: dict) -> None:
    """Trains model.
    ...
    for epoch in range(config["hparams"]["n_epochs"]):
        ...
        for batch_index, (data_student, targets_student) in enumerate(trainloader_teacher):
            data_teacher, targets_teacher = trainloader_student[batch_index]
            ...
            loss, corr = train_single_batch(net, data_student, targets_student, data_teacher, targets_teacher, optimizer, criterion, device)
            running_loss += loss
            correct += corr
        ...
        log_dict = {"epoch": epoch, "time_per_epoch": time.time() - t0, "train_acc": correct/(len(trainloader_student.dataset)), "avg_loss_per_ep": running_loss/len(trainloader_student)}
        log(log_dict, step, config)
